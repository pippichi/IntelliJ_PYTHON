...
from ReadFile import readfile, ReadCsv

class Preprocessor:
    def __init__(self):
        self.features = []

    def process_data(self,fileName,feature_num=20):


        lists = str(self.loan_csv(fileName))
        # process the data
        stopwords = self.loan_txt("corpus\\停用词表.txt")
        pos = self.loan_txt("corpus\\褒义词表.txt")
        neg = self.loan_txt("corpus\\贬义词表.txt")
        final_lists = self.get_seg_list(lists,stopwords)
        pos_seg_list = self.get_seg_list_copy(final_lists, neg)
        neg_seg_list = self.get_seg_list_copy(final_lists, pos)
    ...
    def get_seg_list_copy(self, array, stopwords):
        seg_list = []
        for each in array:
            for i in each:
                local_list = jieba.cut(i, False)
                final_list = []
                for word in local_list:
                    if word not in stopwords and word != ' ':
                        final_list.append(word)
    ...
    def feature_list(self, pos_words_tfidf, pos_feature_num, neg_words_tfidf, neg_feature_num):
        features = []
        for each in pos_words_tfidf[2:pos_feature_num+2]:
            features.append(each[1])
        for each in neg_words_tfidf[2:neg_feature_num+2]:
            features.append(each[1])
        return features

    def create_train_csv(self, features, pos_freq_dist, neg_freq_dist):
        with codecs.open('corpus\\test.csv', 'w', 'utf-8') as f:
...
if __name__ == '__main__':
    ...
    # processor.process_data("C:\\Users\\HASEE\\Desktop\\ai_challenger_sentiment_analysis_trainingset_20180816\\sentiment_analysis_trainingset.csv")
    processor.process_data("C:\\Users\\HASEE\\Desktop\\ai_challenger_sentiment_analysis_validationset_20180816\\sentiment_analysis_validationset.csv")
